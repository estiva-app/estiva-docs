...
| --- | --- | --- | --- |
| 1 | ~~**REM-1**~~ (`9cf225fb`, done, peek#336): the DM view reads and writes the relay through FOL-31's conversation view with a `{ kind: 'channel', h }` container; CON-7's guard lands here | ~~**FOL-26**~~ (`fcddc731`, done, peek#337): no route renders `TopicsPage`, and old links land on files. ~~**FOL-25**~~ (`4e1e70f1`, done, peek#342): the restored channels and 6 of 8 test channels are deleted | **REM-4** (`4764de85`): stars and open work live only on the relay |
