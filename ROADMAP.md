...
`RIC-10`'s Peek half (Ship's merged 2026-09-04) and `RIC-2` finish the inline-mention work. `RIC-13` gives a description field prose and blocks wherever it is drawn, Cmd+K included. `INT-11` (Edge shows Ask rows that can never work). `SHI-4` (refs are not unique) is worth more than its size suggests — it is why every `ship` command takes an address. From [Shrinking Convex](#shrinking-convex): **SHR-1** and **SHR-2** (MS1), **SHR-3** and **SHR-4** (MS3) and **SHR-6** (MS4) are **done** (peek#229, peek#231, peek#232, peek#235, 2026-09-16); **SHR-5** (**MS4**, not MS3 — this line had the number wrong) is the substantive half that is left, and is independent of both the Folders sequence and the DM track, though it is gated on MS2 (SHR-8). **SHR-6 was not gated, and shipped**: the MS2 gate is about content Convex holds the only copy of, and the starred set is ids the relay already had, so taking Convex out of that read lost nothing. It took starred *people* with it — the exclusion had quoted a layer-1 argument at a layer-2 blob. **SHR-9**, filed 2026-09-16 out of SHR-7, is the one MS4 ticket gated on *nothing* — open work is pure curation, it moves to the layer-2 blob whole, and it needs neither SHR-8 nor the DM track. It is the cheapest start available in this project. **MS5** (CRO-10) is no longer gated: MS3 gave it the number it was waiting for — Convex's horizon cache holds 130 of 296 contexts, losing nothing. Then the bug lists in **Peek: Improvements**, **Peek: Feedback & Bugs** and **Ship: Feedback & Bugs**. **One coordination point**: if the conversation components are in the `@estiva-app/ui` migration queue, CON-5 extracts after that lands or the migration happens inside the package — otherwise the package ships the old primitives on day one.
...
| **MS4** | Take the topic half off Convex — SHR-5 (the largest *code* win) and SHR-6 (**done** 2026-09-16, peek#235), joined 2026-09-16 by **SHR-9** (open work onto the layer-2 blob) and **SHR-10** (the Screener as a fold) | **Gated on MS2 — except SHR-9 and SHR-6.** SHR-9 is gated on nothing at all and is the cheapest way in; SHR-6 was not gated either, because the gate is about content Convex holds the only copy of and a set of ids is not that. SHR-5 and SHR-10 split cleanly from the DM branch; SHR-6 did not have to, and took starred people too |
