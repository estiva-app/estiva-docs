...
`RIC-10`'s Peek half (Ship's merged 2026-09-04) and `RIC-2` finish the inline-mention work. `RIC-13` gives a description field prose and blocks wherever it is drawn, Cmd+K included. `INT-11` (Edge shows Ask rows that can never work). `SHI-4` (refs are not unique) is worth more than its size suggests — it is why every `ship` command takes an address. From [Shrinking Convex](#shrinking-convex): **SHR-1** and **SHR-2** (MS1), **SHR-3** and **SHR-4** (MS3) and **SHR-6** and **SHR-9** (MS4) are **done** (peek#229, peek#231, peek#232, peek#235, peek#238, 2026-09-16/17); **SHR-5** (**MS4**, not MS3 — this line had the number wrong) is the substantive half that is left, and is independent of both the Folders sequence and the DM track, though it is gated on MS2 (SHR-8). **SHR-6 was not gated, and shipped**: the MS2 gate is about content Convex holds the only copy of, and the starred set is ids the relay already had, so taking Convex out of that read lost nothing. It took starred *people* with it — the exclusion had quoted a layer-1 argument at a layer-2 blob. **SHR-9** is **done** (peek#238, 2026-09-17) — open work is pure curation, it moved to the layer-2 blob whole, DM entries included, and it needed neither SHR-8 nor the DM track, exactly as SHR-7 predicted. It did **not** copy the `stars` blob verbatim: open work is an ordered list a person will be able to reorder, so it is one `entries` array rather than two sets (see [`decisions/SHR-7-screener-and-desk.md`](decisions/SHR-7-screener-and-desk.md) §1). **SHR-10** is now the cheapest start available in this project. **MS5** (CRO-10) is no longer gated: MS3 gave it the number it was waiting for — Convex's horizon cache holds 130 of 296 contexts, losing nothing. Then the bug lists in **Peek: Improvements**, **Peek: Feedback & Bugs** and **Ship: Feedback & Bugs**. **One coordination point**: if the conversation components are in the `@estiva-app/ui` migration queue, CON-5 extracts after that lands or the migration happens inside the package — otherwise the package ships the old primitives on day one.
...
**Why DMs are excluded.** They block one table of fifteen (`dmConversations`) plus a DM branch inside five modules — not the programme. The DM track is eleven tickets, of which **four are done as of 2026-09-17** — DMS-1 to DMS-4, so a DM has a channel on the relay and its messages, replies, edits and deletions travel (peek#240, peek#246) — and the remaining seven are ungated. DM-coupled work stays in *DMs on Nostr*. So *zero messages in Convex* is the right end state and is **not reachable inside this project alone**; it needs *DMs on Nostr* finished too. This project gets every message that is not a DM.
...
| **MS4** | Take the topic half off Convex — SHR-5 (the largest *code* win) and SHR-6 (**done** 2026-09-16, peek#235), joined 2026-09-16 by **SHR-9** (open work onto the layer-2 blob, **done** 2026-09-17, peek#238) and **SHR-10** (the Screener as a fold) | **Gated on MS2 — except SHR-9 and SHR-6.** SHR-9 was gated on nothing at all and shipped; SHR-10 is now the cheapest way in; SHR-6 was not gated either, because the gate is about content Convex holds the only copy of and a set of ids is not that. SHR-5 and SHR-10 split cleanly from the DM branch; SHR-6 did not have to, and took starred people too |
