...
| 3: after REM-3 (landed) and FOL-23 | ~~**REM-6**~~ (`48378c5e`, done, peek#350): the viewer is a pubkey. The DM lane is finished | ~~**CON-14**~~ (`083b091a`, done, peek#360): the Desk reads the relay. Its Urgent leg moved to **CON-17** (`d8eeda32`), because urgency has had no wire form since RIC-2 | **REM-5** (`6316e113`): read state is the blob only (replaces the cancelled CRO-10) |
