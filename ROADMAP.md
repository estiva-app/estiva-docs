...
| **Manifests: every app declares its actions** | Opened 2026-09-25 (`f80c46dd`, Estiva HQ). Every app declares in its `kind:31990` every action another app may take, and reads other apps' objects only through manifests. Ship declares 6 of the ~17 things it does. **MAN-1** (SPEC §7 vocabulary: move field, placement, document value, reply parent) unblocks most; **MAN-4**'s first four actions need nothing; **MAN-3** makes Folders protocol-level and settles one name, *Folder* — "team" goes. **PER-21** (the agent drops its copy of Ship's code) waits on this project and on PER-20 |
