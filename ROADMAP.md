...
| **then** | ~~SHR-8 migrates 145 as `1111` on files~~ (**done 2026-09-22**, peek#294 — 145 published, 0 refused, all dated and threaded, census 202 → 57) → CON-12 copies the 26 partials and settles the 481, which is now the **only** thing between here and a census of 0 → census verifies → CON-6 (`a61e921f`) retracts **the 134** → **B6: CON-7 census 0 *and* CON-12's probe 0** → CON-8 (`19ae5b48`) Peek stops writing messages to Convex (**deployed 2026-09-23**: peek#304 made topics read-only, peek#309 disabled huddles and New topic / DM Start topic; census 08:16Z read 0 to migrate of 989, and needs to read 0 again on or after 2026-09-24 08:16Z) → ~~CON-9 (`209fa791`) counts the DM side~~ (**done 2026-09-23**, peek#320 — from a `convex export`, since a token sees only its holder's pairs: **166** DM rows never published, all Katerina ↔ Miky 2026-07-17 → 09-17, 20 attachments, 0 recorded ids missing on the relay) → DMS-7 (`507bf8c2`) republishes them, option 1 (Miky, 2026-09-22), then Convex is shut down |
...
| **What happens to Convex-only DMs** | DMS-7 | Decided 2026-09-22: republish (option 1), with true dates through the floor window CON-13 made possible. Sized by CON-9: 166 messages, 20 attachments, 14 reactions, one pair. In the migration project |
