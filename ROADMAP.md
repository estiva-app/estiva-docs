...
| **then** | ~~SHR-8 migrates 145 as `1111` on files~~ (**done 2026-09-22**, peek#294 — 145 published, 0 refused, all dated and threaded, census 202 → 57) → CON-12 copies the 26 partials and settles the 481, which is now the **only** thing between here and a census of 0 → census verifies → CON-6 (`a61e921f`) retracts **the 134** → **B6: CON-7 census 0 *and* CON-12's probe 0** → CON-8 (`19ae5b48`) Peek stops writing messages to Convex (**deployed 2026-09-23**: peek#304 made topics read-only, peek#309 disabled huddles and New topic / DM Start topic; census 08:16Z read 0 to migrate of 989, and needs to read 0 again on or after 2026-09-24 08:16Z) → CON-9 (`209fa791`) counts the DM side with a signed-in token (peek#258's mechanism) → DMS-7 decides it |
