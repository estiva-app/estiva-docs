...
Last updated 2026-09-20. Not published to the docs site (`site/nav.mjs` is opt-in) because it changes often and carries operational detail.
...
| A6 | from FOL-27 (struck 2026-09-18) | **Blocking A7 beside A3:** **FOL-35** (the team's general `kind:9` chat as a row — `/topic/<team uuid>` stops resolving with `TopicsPage`), **FOL-36**'s `/message/<id>` half (a link to a file comment lands by `h` on the team channel — dead once `TopicsPage` goes), **FOL-14** (a way to start a topic). **Not blocking:** ~~FOL-32~~ (live `1111` — shipped in peek#266, 2026-09-19, verified with two browsers), ~~FOL-33~~ (rename/delete a bare file — shipped in peek#267 on interop 0.25.0, 2026-09-19; the bare file's built-in manifest now declares `rename` and `delete`, and a deletion is an action shape), FOL-34 (Screener, Desk, Open work never see a `1111`), FOL-36's search/Recent half, CON-11, HIG-1. Found on the way: **PRO-21** — a Peek comment on a re-linked Ship project lands in the record's Folder and Ship reads the re-linked one, so it is invisible in Ship; a projection-layer bug, not a Topics one |
| A7 | **FOL-26** | **Retire the Topics view.** Gated on A2's checklist — struck 2026-09-18: A3, FOL-35, FOL-36's `/message` half and FOL-14 — and on **B6** — the Topics view is the only reader of **184** messages (a live census reading; 202 when the project opened) and 62 attachments |
...
The project brief on Ship is the source of truth for steps 0–8. What 2026-09-18 decided: **(a) the floor is lowered for the window**, not a date-in-a-tag rule every reader carries for ever — the unpublished ones sit between 653 messages with true dates, so a tag would have every consumer sorting by it. **The keys are exported** (Miky's and Katerina's, by Miky, into a file the repo ignores before it exists), so the migration does not depend on Katerina's availability. **The FOL-25 copies are re-done under the same mechanism** — nothing obsolete stays.

**The census is a live reading, not a constant** (CON-3, peek#260, 2026-09-18). It was 202 unpublished of 807 when the project opened; today the same probe — and `probe-shr8-census.ts` re-run in the same minute — reports **184 of 837**, with 605 → 653 published and the 17 unreadable unchanged. Two things follow for sizing. Publication cannot be what moved the unpublished count: `MAX_DRIFT_MS` is 15 minutes (`convex/nostr/refs.ts`), so a row that was null at the census can never become published later, and those 18 rows left Convex between the runs. And the discriminator the retraction needs is the **`orig` tag**, not a `ts`: 431 events carry `ts` without it — Ship's comments — and the copies carry no `ts` at all, so a `ts` rule would retract 431 correct events and miss every copy. Re-read the count before sizing anything against it.
...
- **FOL-26 and everything behind it** (FOL-25 delete mode, FOL-23, C's deletions) — the Topics view is the only reader of 184 messages and 62 attachments. Until the census reads 0 (B6) retiring it loses content silently; that is the shape PEE-30 already produced once.
...
| blocked | by | why |
| --- | --- | --- |
| **FOL-26** (retire the Topics view) | **CON-7** census 0 · Topics-parity | A message sent in the Topics view after its channel is re-deleted lands in Convex with a null `nostrEventId` and reaches nobody; and 202 messages are readable nowhere else today |
