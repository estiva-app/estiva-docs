...
| **Convex → Buzz data migration** | Opened 2026-09-17 out of SHR-8's census; **`in_progress` and the whole critical path since 2026-09-22**, now that phase A is done. **Move every non-DM message and attachment out of Convex into Buzz with its real date, and leave one store behind.** Fourteen tickets — FOL-25 moved to *Remove Convex* on 2026-09-22, its migration half done and only the channel deletions left, which are phase C. The sequence, the four decisions of 2026-09-22 and the measured bucket sizes are in the project brief on Ship, which is the source of truth. The premise: Convex is **not a mirror** — it was the *only* copy of **202 of 842** non-DM messages and **62 of 68** attachments, across 26 topics and a huddle. **SHR-8 ran on 2026-09-22** (peek#294) and published 145 of them onto their `30840` files with their true `created_at`, taking the message half to **57 of 844** — 23 of which are CON-4's deliberate discard and 34 of which are CON-12's two live record channels. The attachment half is untouched at **62** and is CON-5's, because the census scores an attachment on the `imeta` describing it, not on its message being readable. **The count is a reading, not a constant** (peek#244 opened this at 202 of 807; 2026-09-18 read 184 of 837). **On 2026-09-23 the census reads 0 and `probe-con16` reads 0** (one relay-only reply accepted, peek#325). Miky dropped CON-7's second reading, and CON-7's guard is blocked on a relay-first DM send that no ticket tracks yet. Code deletion is not here; that is FOL-23/FOL-26 |
...
**The agent's ceiling moved and this document was the last to know.** Read directly on 2026-09-22 against `id.estiva.app/sign`, nothing published: `9`, `1111`, `30840`, `9008`, `5`, `1852` all answer **200**; `1` and `24242` answer **422**. ~~Only blob authorization is still withheld.~~ **`24242` was granted to `claude-agent` on 2026-09-23 (CON-5)**, that row only. Every earlier line here claiming the agent lacks `30840`, `1852` or `9008` was stale — read the ceiling, never recall it.

**The census is a live reading, not a constant** (CON-3, peek#260). 202 of 807 when the project opened; **184 of 837** on 2026-09-18; **202 of 842** on 2026-09-22 (185 never published + 17 recorded-but-unreadable); **57 of 844** later the same day, once SHR-8 ran. It moved in both directions while nobody was publishing, and one new unpublished row appeared on 2026-09-21. Before SHR-8, publication was not what moved it — `MAX_DRIFT_MS` is 15 minutes (`convex/nostr/refs.ts`), so a row that was null at a census can never become published *by Peek* later; rows *left* Convex instead. A migration run moves it because it writes `nostrEventId` back itself. Re-read the count before sizing anything against it.
...
**Done 2026-09-23 (CON-6, peek#319): the 134 are retracted, and `orig` stopped being the discriminator before they were.** CON-5's `shr8-migrate` writes `orig` too, so it now marks 461 events, 289 of them good copies Convex points at. Select by `['migration', 'fol25-remigrate']` instead. None of the 134 had a true-dated twin (`shr8-migrate` had counted each as done), so the run republished 137 rows (the 134 plus 3 replies threaded under them) and then retracted the old copies. Afterwards: 0 duplicates, 0 misdated, bucket 1 unchanged at 645, bucket 3 back at 17. What follows is the 2026-09-22 reading.
...
| who | what |
| --- | --- |
| **Miky** | the key export; ~~a `kind:24242` grant for the blob re-upload~~ (granted 2026-09-23; all 62 blobs uploaded, peek#312); and **one signed-in click** on a project-record channel's conversation for CON-12's 481. ~~The env-var read~~ — **CON-2 is done, 2026-09-22** (below) |
| **agent, now** | the snapshot (CON-10 / `05c2773d`) **first** — content is being destroyed; ~~the buzz PR making `CREATED_AT_FLOOR_SECS` env-overridable~~ (done and deployed, buzz#19 / CON-13); ~~CON-4's missing `30840`~~ (done, peek#291); CON-7's guard (`706eb704`); the migrate, copy and retract scripts |
| **then** | ~~SHR-8 migrates 145 as `1111` on files~~ (**done 2026-09-22**, peek#294 — 145 published, 0 refused, all dated and threaded, census 202 → 57) → CON-12 copies the 26 partials and settles the 481, which is now the **only** thing between here and a census of 0 → census verifies → ~~CON-6 (`a61e921f`) retracts **the 134**~~ (**done 2026-09-23**, peek#319: republished 137 true-dated, then retracted) → **B6: CON-7 census 0 *and* CON-12's probe 0** → CON-8 (`19ae5b48`) Peek stops writing messages to Convex (**deployed 2026-09-23**: peek#304 made topics read-only, peek#309 disabled huddles and New topic / DM Start topic; census 08:16Z read 0 to migrate of 989, and needs to read 0 again on or after 2026-09-24 08:16Z) → CON-9 (`209fa791`) counts the DM side with a signed-in token (peek#258's mechanism) → ~~DMS-7 decides it~~ (**done 2026-09-23**, peek#324: option 1, 166 rows + 10 reactions + 1 resolution republished true-dated as their authors, 17 blobs uploaded; census DM arm 0; 175 Convex copies imported mid-run stay until shutdown) |

### C — Remove Convex

A new project (2026-09-18, `e6aefcdc`) for what *Shrinking Convex* excluded and the migration does not cover: the residue and the deletion. Its brief maps all fifteen tables to their relay homes. Three small decisions first (rename history — Buzz applies a name change silently; `isExternal`/`role`; the one huddle), then the per-person blobs (screener snooze/dismissal; CRO-10 re-scoped to *drop the read-state cache*), then the two relay reads (my DM channels; the People directory from the roster), then — after B6 — FOL-25 delete mode, FOL-23, the dead tables, `convex/` gone, the deployment deleted behind a snapshot. **Huddles** are out of scope by their own placeholder project (`cec874b6`): **disabled 2026-09-23** (peek#309) — existing ones stay readable, none can be started or replied to; the rebuild is HUD-1 (`a7a301a9`), not scheduled.
...
- **FOL-26 and everything behind it** (FOL-25 delete mode, FOL-23, C's deletions) — the Topics view is the only reader of what is left, now **57** messages rather than 202, plus 62 attachments and CON-12's 507. Until the census reads 0 (B6) retiring it loses content silently; that is the shape PEE-30 already produced once.
- **The migration's execution** — from a person: ~~the key export~~ (done 2026-09-22, used by SHR-8 and shredded after the run), ~~a `24242` grant or a hand-run of the blob script~~ (granted 2026-09-23), and one signed-in click for CON-12's 481. **The env-var read and the un-delete both came off this list on 2026-09-22.** The agent's ceiling now covers `30840`, `1852`, `9008` and `5`; only `24242` is withheld. A note on how the gate stayed shut for four days: the `ssh` read was reported blocked by the classifier, and the allow rule had been in `settings.local.json` all along — the command began `timeout 25 ssh …`, and an allow rule is a prefix match on the whole command. **Check the allow list before calling a box read impossible.**
...
| ~~**everything in B**~~ | ~~**CON-2**~~ | **Answered 2026-09-22: production reads no replica**, so the floor comes down for the window through the normal ingest path. `READ_DATABASE_URL` is unset in `/opt/buzz/.env`, in `docker-compose.yml` and in the running container, and the relay logged `"Postgres connected"` — `main.rs:155` prints that or `"(writer + read replica)"` off `has_read_pool()`. Migration 0021's no-GUC route is not needed, so its derived-write repair (`thread_metadata` and the mention index for 141 replies) never has to be scoped. **`BUZZ_REPLICA_READ_MAX_AGE_MS` does not exist in the codebase** — the mechanism is one variable, not two |
| Migrate (**SHR-8**) | **CON-2** · ~~CON-4 un-delete~~ · ~~**CON-5** re-upload 62 blobs~~ (**done 2026-09-23**: bytes peek#312, 52 author-signed `imeta` edits peek#317, readers ship#172 / peek#316 / interop 0.30.0 per RFC 0.4 §7.2.1 as amended by estiva-docs#146; census 68 of 68 described) · key export | Publish before deleting; the attachment bytes are in Convex `_storage`, and deleting Convex destroys them. **The un-delete dropped out on 2026-09-22** — a `1111` on a file carries `h = <team channel>`, so a hidden topic channel cannot swallow the copy |
...
- **A republished message carries today's `created_at` — unless the window is open, and since CON-13 it can be.** Buzz floors `created_at` at 960 s for anything with an `h` (part of the replica-read correctness proof), and the ingest drift check (`BUZZ_MAX_TIMESTAMP_DRIFT_SECS`, default 900) refuses first with a `400`. Widening only the drift var moves the failure to an opaque `500` rather than lifting it. **Shipped and deployed 2026-09-22** (buzz#19, CON-13): the floor is `BUZZ_CREATED_AT_FLOOR_SECS`, default 960 unchanged, and **both** vars go in `/opt/buzz/.env` for the window — see `operations/PRODUCTION.md` § *Opening the back-dating window*. Measured on production that day: with the floor at 7200 a 3600 s-old `kind:9` was accepted and read back, an 86400 s-old one was refused, and with the floor restored the same event was refused again (probe `peek/scripts/probe-con13-floor-knob.ts`, peek#293). Both vars were removed afterwards; **the window is closed and each migration run re-opens it**. The proviso held: CON-2 found no read pool on production, so the proof the floor defends guards nothing in use. A date-in-a-tag rule was rejected: the unpublished messages sit between 653 with true dates, so every reader would carry the sort rule for ever.
