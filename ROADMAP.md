...
**Verified by a person on 2026-09-02, and closed.** Two clauses stay undemonstrated for a structural reason rather than an unfinished one: PEE-2 and PEE-4 both promise that somebody who *cannot read* an object sees the absence honestly, and production has no private Folder to be refused from — the same gap [RFC 0.4](protocol/RFC-0.4-WORKSPACE.md) §5.2 already tracks. Implemented and asserted in tests; no environment to exercise them.

**A follow-on track is filed as `INT`** — asking, drafting and proposing an action from inside the launcher, matched over whatever any app's manifest declares rather than over anything Ship-specific. Ship carries what is open; do not infer it from here.
