...
**What acceptance does not settle.** Every item in §12 stays open, and accepting this document does not decide any of them. Four carry tickets: **which content format** (§14.5, RIC-1), ~~the reaction horizon~~ (§7.2, CON-1 — **answered 2026-09-07**, see question 10), **upstream or fork** (§10.1, whose *latest* responsible moment is still the first line of folder command or state code), and **the folder kind numbers** (§12.1, which §10.1 governs). Component anchoring is *unfrozen, not chosen* — §6's three candidate mechanisms are all still live and want a real editor to choose between.
...
**Reactions are `kind:7`, and they have a query shape worth knowing before you build one.** SPEC did not mention reactions at all until 0.4's adoption added [SPEC §6.6](SPEC.md), which records the mechanism below as ratified fact. One property makes them harder than they look: **a `kind:7` carries no `h` tag**, so reactions cannot be found the way messages are. They are fetched addressed by *target* rather than by channel or cursor, capped at 100 targets (`REACTION_TARGET_LIMIT` in both apps). An app MUST NOT assume a channel query returns reactions.

*Amended 2026-09-07.* This paragraph called the cap "undocumented" and left the horizon to each app. Both are now settled — [SPEC §6.6](SPEC.md) fixes N at 100, requires a client to report truncation, and requires one shared budget across id spaces. That closes open question 10.
...
~~**Today only Peek implements any of this.**~~ *Superseded 2026-09-07.* It said Ship had no reactions, no edit and no delete on comments, and that a grep for reaction handling in `estiva-ship` returned nothing. Ship now has **reactions** (CON-2) and **delete** (CON-3), both built to the corrected §6.5 rule — the control is offered to everyone and the relay adjudicates. **Edit remains unbuilt in both apps** and is CON-4; Peek's own edit is a local patch that publishes nothing, which is CON-8. So the sentence to carry forward is narrower and still true: an app that never built these needs §13's projection layer, and the two that did had to agree first — which is what CON-1's horizon and §6.5's correction were each about.
