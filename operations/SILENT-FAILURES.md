...
| You added `h` to a deletion so the relay could scope it | **`kind:5` derives its channel from the target and ignores a supplied `h`.** It is not in `requires_h_channel_scope`; ingest looks the target up. Harmless, but a comment claiming the tag is required sends the next person looking for a rule that is not there. **`kind:40003` is the opposite** and genuinely requires one | Read `requires_h_channel_scope` in `ingest.rs` — it is a *different list* from the kind allowlist |
